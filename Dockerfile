...
# README.md is included because pyproject.toml declares it as the readme, and
# the build backend reads it during `uv sync` - before `COPY . .` runs.
COPY pyproject.toml uv.lock README.md ./
# This project is a package (uv_build backend), so `uv sync` needs an importable
# module to exist before the real source is copied in. A stub satisfies the build
# backend; `COPY . .` below replaces it with the real package, and because the
# install is editable it resolves to the real files at runtime.
RUN mkdir -p src/voice_agent && touch src/voice_agent/__init__.py
...
# The generated default ran the module file directly, which only imports it and
# exits: this project uses the AgentServer API and has no __main__ block. The
# module CLI discovers the `server` object in agent.py and actually starts it.
CMD ["uv", "run", "python", "-m", "livekit.agents", "start", "agent.py"]
